...
class MainWindow():
    ...
    subject = 'F1' # AH, CB, DG, JR, LT, F1, F2
...
#    print(np.unique(separated_arrays[...,2],return_counts=True))
    ...
    bg_coord_lists = np.empty((z_size,),dtype=object)
...
#            plt.figure()
#            plt.imshow(self.img_array[:,:,20,20], cmap='Greys')
#            plt.show()
#            print(self.img_array.shape)
#            print(np.amin(self.img_array))
#            print(np.unique(self.img_array,return_counts=True))
    ...
    def run_seg(self):
        self.label1.config(text='==== RUNNING 1/3 ====')
        time.sleep(5)
        self.run_random_walker3D()
        
        self.label1.config(text='==== RUNNING 2/3 ====')
        time.sleep(5)
        self.load_seg_mark()
        
        self.label1.config(text='==== RUNNING 3/3 ====')
        time.sleep(5)
    ...
    def __init__(self, main):
        ...
        self.button0 = Button(main, text='Run', width=20, command= lambda : self.run_seg())
