...
def _make_graph_edges_4d(n_x, n_y, n_z, n_t):
...
def _compute_weights_4d(data, spacing, eps=1.e-6,alpha=0.3, beta =0.3, gamma=0.4, a=130.0, b=10.0, c=800.0):
...
def _compute_intensity_weight_4d(data, spacing,eps=1.e-6, a=130.0):
...
#    print(delta_I_sq.shape)
    intensity_weights = np.exp(- delta_I_sq)
    intensity_weights += eps
    return intensity_weights

def _compute_magnitude_weight_4d(data, spacing,eps=1.e-6, b=10.0):
    #split the calculation of the exponentials of the intensity, velocity magnitude and velocity direction part
    #calculate intensity part here
...
def _compute_delta_I_4d(data, spacing, a):
...
#    print(data.shape)
    ...
    gr_deep *= a/data[...,0].std()
    gr_down *= a/data[...,0].std()
    gr_right *= a/data[...,0].std()
    gr_time *= a/data[...,0].std()
...
def _compute_eucl_dist_4d(data, spacing, b):
    ...
    dist_y_time = np.abs(data[:, :, :, :-1, 2] - data[:, :, :, 1:, 2]).ravel() / spacing[3]
    ...
    dist_z_time = np.abs(data[:, :, :, :-1, 3] - data[:, :, :, 1:, 3]).ravel() / spacing[3]
...
#    print('data[...,1:].std,: '+str((data[...,1].std)))
    ...
    dist_down *= b/data[...,1:].std()
    dist_time *= b/data[...,1:].std()
